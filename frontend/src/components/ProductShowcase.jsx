...
export default function ProductShowcase({ products }) {
  ...
  return (
    ...
    >
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 sm:pt-14 pb-14">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-6 sm:mb-10">
          <div>
            ...
            <h1 className="mt-2 font-serif-display text-4xl sm:text-5xl lg:text-6xl font-light text-[#3D2A3A] leading-[1.05]">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              className="h-11 w-11 rounded-full bg-white/70 backdrop-blur border border-[#F4DBD8] hover:bg-white text-[#3D2A3A]"
              aria-label="Anterior"
            >
        ...
        <div className="overflow-hidden" ref={emblaRef}>
          <div className="flex -mx-3">
            {featured.map((p) => (
              <div
                key={p.id}
                className="min-w-0 shrink-0 grow-0 basis-full sm:basis-[70%] md:basis-[55%] lg:basis-[45%] xl:basis-[40%] px-3"
                data-testid={`showcase-slide-${p.code}`}
              >
                <article className="grid grid-cols-1 sm:grid-cols-[1.1fr,1fr] gap-4 sm:gap-6 rounded-3xl bg-white border border-[#F4DBD8] overflow-hidden shadow-[0_30px_80px_-40px_rgba(61,42,58,0.35)]">
                  ...
                  <div className="p-5 sm:p-6 flex flex-col justify-between">
                    <div>
                      ...
                      <h3 className="mt-1 font-serif-display text-3xl sm:text-4xl text-[#3D2A3A] leading-tight">
                        {p.name}
                      </h3>
                    </div>
                    <div className="mt-6">
                      <span className="block font-serif-display text-5xl sm:text-6xl font-light tracking-tight text-[#D48A94]">
                      ...
                      <Button
                        ...
                        className="mt-5 rounded-full h-11 px-6 bg-[#3D2A3A] hover:bg-[#2C1F2A] text-white gap-2 disabled:opacity-40"
                      >
        ...
        {/* Dots */}
        <div className="flex items-center justify-center gap-2 mt-6" data-testid="showcase-dots">
          {featured.map((_, i) => (
            <button
              key={i}
              onClick={() => scrollTo(i)}
